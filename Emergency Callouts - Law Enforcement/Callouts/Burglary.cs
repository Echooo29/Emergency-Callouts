    ...
    public class Burglary : Callout
    {
                ...
                GameFiber.StartNew(delegate
                {
                    while (CalloutActive)
                    {
                        ...
                        if (PedFound)
                        {
                            ...
                            Game.LogTrivial("[Emergency Callouts - Law Enforcement]: Set pursuit active for player");
        ...
        private void Scenario4() // Attack
        {
            ...
            try
            {
                ...
                Game.LogTrivial($"[Emergency Callouts - Law Enforcement]: Assigned (WEAPON_CROWBAR) to Suspect inventory");
        ...
        private void Scenario5() // Putting Items In Vehicle
        {
            ...
            try
            {
                ...
                Game.LogTrivial("[Emergency Callouts - Law Enforcement]: Retrieved vehicle position");

                // Suspect Resistance Chance
                Functions.SetPedResistanceChance(Suspect, 40f);
                Game.LogTrivial("[Emergency Callouts - Law Enforcement]: Set ped resistance chance to 40%");
                ...
                GameFiber.StartNew(delegate
                {
                    while (CalloutActive)
                    {
                        ...
                        if (MainPlayer.Position.DistanceTo(Suspect.Position) < 15f && Suspect.Exists())
                        {
                            ...
                            // Set Pursuit Active
                            Functions.SetPursuitIsActiveForPlayer(pursuit, true);
                            Game.LogTrivial("[Emergency Callouts - Law Enforcement]: Set pursuit is active for " + PlayerPersona.FullName);
